...
/*    const dispatch = useDispatch(); */
  ...
  const resultNumber = Number(resultInPercents? resultInPercents.slice(0, -1) : 0);
  const incorrectNumber = 100 - resultNumber;
   ...
   const correctAnswers = parseInt(((totalQuestions*resultNumber)/100), 10);
    ...
    return (
      ...
      <div><Diagram  data= {[['Answer', 'Percentage'], ['Correct', resultNumber], ['Incorrect', incorrectNumber]]}/></div>
     ...
     <img src={resultsImg}
      ...
      className={s.button}
      type="button"
      onClick={backToTestPage}> Try again 
      </button>
